...
{
    var kameraYgDipilih by rememberSaveable{ mutableStateOf("")}
    ...
    Column (modifier = modifier,
        ...
        Modifier.padding(dimensionResource(R.dimen.padding_medium))){
            pilihanKamera.forEach{ item->
                Row(modifier = Modifier.selectable(
                    selected = kameraYgDipilih == item,
                    onClick = {
                        kameraYgDipilih = item
                        onSelectionChanged(item)
                    }
                ),
                    verticalAlignment = Alignment.CenterVertically){

                    RadioButton(selected = kameraYgDipilih == item,
                        onClick = {
                            kameraYgDipilih = item
                            onSelectionChanged(item) }
                    )
